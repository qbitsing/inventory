...
.controller('HomeCtrl', function ($scope) {
    ...
    ws.onmessage = function(message) {
    	...
    	if (obj.result) {
            ...
            /*else if($scope.operacion='7'){
                titulo="Registro exitoso";
                mensaje+='<li>La llave programadora esta lista para transmitir información</li>';
            }*/
            mensaje+='</ul>';
            swal({
                title: titulo,
    ...
    $scope.crearCandado=function(){
        if ($scope.conexion) {
            $scope.operacion='8';
            sendRequest({"cmd": "createsettingkey", "data": {"syscode": "36363636","regcode": "31313131", "id": 5, "newlockid": 26, "newlockgroupid":333,"beep":true}});
        }else{
            sweetAlert("Oops...", "No se ha podido hacer conectar con el socket" , "error");
        }
    }
