...
let buscar = co.wrap(function * (req, res){
	try{
		...
		let entradasRemision = yield entradasRemisionModel.find({fecha : { $gte: fechaInicial, $lte: fechaFinal }});
		let salidasFabricacion = yield salidasFabricacionModel.find({fecha : { $gte: fechaInicial, $lte: fechaFinal }});
